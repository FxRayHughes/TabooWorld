package ray.mintcat.tabooworld.utils
...
fun toInfo(sender: CommandSender, message: String) {
    sender.sendMessage("§8[§a TabooWorld §8] §7${message.replace("&", "§")}")
...
fun toError(sender: CommandSender, message: String) {
    sender.sendMessage("§8[§4 TabooWorld §8] §7${message.replace("&", "§")}")
    if (sender is Player && !cooldown.hasNext(sender.name)) {
...
fun toDone(sender: CommandSender, message: String) {
    sender.sendMessage("§8[§6 TabooWorld §8] §7${message.replace("&", "§")}")
    if (sender is Player && !cooldown.hasNext(sender.name)) {
...
fun toConsole(message: String) {
    Bukkit.getConsoleSender().sendMessage("§8[§e TabooWorld §8] §7${message.replace("&", "§")}")
}
